...
def scr_1(spec_in,spec_dat,max1,mass_tol):
    arc=0
    are1=[]
    are2=[]
    max_in=ret.in_peaks3(spec_in[0],spec_in[1],0.001)
    max_dat=ret.in_peaks3(spec_dat[0],spec_dat[1],0.001)
    for a1 in max1:
        #print(a['mass'])
        adm=[]
        for b in max_in:
            #print(a,b['mass'],isinstance(b['mass'], dict))
            dm=a1-b['mass']
            adm.append(dm)
            if dm < 0:
                break
        min_value = min(adm)
        min_index1 = adm.index(min_value)
        if min_value < 0.5:
            x1=max_in[min_index1]['mass']-mass_tol
            x2=max_in[min_index1]['mass']+mass_tol
            range1=[x1,x2]
        elif min_value >  0.5:
            range1=[]
        ...
        min_index1 = adm.index(min_value)
        if min_value < 0.5:
            x1=max_dat[min_index1]['mass']-mass_tol
            x2=max_dat[min_index1]['mass']+mass_tol
            range2=[x1,x2]
        elif min_value >  0.5:
            range2=[]
        ...
        if len(range1)==0:
            ...
            ar1=area1(spec_in[0],spec_in[1],range1)

        if len(range2)==0:
            ar2=0
        elif len(range2)==2:
            ar2=area1(spec_dat[0],spec_dat[1],range2)
...
def scr_2(spec_in,spec_dat,max1,mass_tol):
    arc=0
    are1=[]
    are2=[]
    max_in=ret.in_peaks3(spec_in[0],spec_in[1],0.001)
...
def scr_3(spec_in,spec_dat,max1,mass_tol):
    arc=0
    are1=[]
    are2=[]
    max_in=ret.in_peaks3(spec_in[0],spec_in[1],0.001)
